...
$Set Scn BLN
$Set Sens BLN
$Set Sens_name HTN_NEP
$Set TNS 0.45      //Transmission capacity
$Set TOU 0.97   //Electricity price
//Set definition
Sets t      / h0 * h%tl% /
     h(t)   / h1 * h%tl% /
     co     / a, b /
...
$Set Sfolder /dssg/home/acct-zmliu/zmliu/Jiaxin_Ding_Conference_MeOH/Data/288_Re_run_Fix/Sensitivity/
$Set Gfolder /dssg/home/acct-zmliu/zmliu/Jiaxin_Ding_Conference_MeOH/Data/288_Re_run/Sensitivity/
$ontext
...
$call csv2gdx %Dfolder%CAPEX_%Sens%.csv  id = Tech_CAP  index = 1  values = 2..lastCol  useHeader = y
$gdxin CAPEX_%Sens%.gdx
$load Tech_CAP
$gdxin
...
$gdxin %Gfolder%S_%EL%_%YS%_ED%ED%.gdx
...
*全国集成度为50%
eq_MSR_Gas .. sum((plt(js,i),h), nds * M_RE('Gas',js,i,h)) =E= DoI * sum((plt(js,i),h),  nds * M_MSR('Gas',js,i,h));
...
Execute_Unload '%Sfolder%S_%EL%_%Sens_name%.gdx' yre, DOI, U_nom, N_modules, P_PV, P_WT, En_ES, ChgP, DisP, P_city_PV, P_city_WT, P_city, P_exp_PV, P_exp_WT, P_exp, P_imp
