...
*全国集成度为50%
eq_MSR_Gas .. sum((plt(js,i),h), nds * M_RE('Gas',js,i,h)) =l=  sum((plt(js,i),h),  nds * M_MSR('Gas',js,i,h));
...
RLCCE_prov(js)$(DOI_prov(js)>0) = (sum((i,h), nds * PV_CO2 * P_PV.l(js,i,h)) + sum((i,h), nds * WT_CO2 * P_WT.l(js,i,h)) + sum((i,h,yre), CF_CO2(js,yre) * nds * P_grid.l(js,i,h))- sum((i,h), nds * M_RE.l('CO2',js,i,h)) + sum((i,h),nds * CO2_em.l(js,i,h)))/sum(i, MeOH_Cap_yr(js,i) * ths);
RLCCE = (sum((js,i,h), nds * PV_CO2 * P_PV.l(js,i,h)$plt(js,i)) + sum((js,i,h), nds * WT_CO2 * P_WT.l(js,i,h)$plt(js,i)) + sum((js,i,h,yre), CF_CO2(js,yre) * nds * P_grid.l(js,i,h)$plt(js,i))- sum((js,i,h), nds * M_RE.l('CO2',js,i,h)$plt(js,i)) + sum((js,i,h),nds * CO2_em.l(js,i,h)$plt(js,i)))/sum((js,i), MeOH_Cap_yr(js,i) * ths);

Display RLCCE_prov, RLCCE;


Execute_Unload '%Sfolder%S_%EL%_%YS%_Opt.gdx' yre, DOI, U_nom, N_modules, P_PV, P_WT, En_ES, ChgP, DisP, P_city_PV, P_city_WT, P_city, P_exp_PV, P_exp_WT, P_exp, P_imp
                                     U_city_nom, F_exp, F_imp, F_prov, P_grid, P_EL, N_EL_on, M_EL, M_RE, M_MSR, MeOH_coal_hr
                                     ...
                                     LCOM, LCOM_prov, RLCCE, RLCCE_prov, DOI_optimized;