...
Parameter TAC_margin;
$gdxin %Gfolder%S_%EL%_%YS%_ED%ED%.gdx
$load  TAC_margin = TAC.l
$gdxin
TAC.m = TAC_margin;
Display TAC.m
...
eq_E_Purs(plt(js,i)) .. E_Purs(js,i) =E= sum(h, nds * tou(js,h) * 1.02249**(%yl%) * P_grid(js,i,h)); //$/kWh -> k$/MWh
...
Execute_Unload '%Sfolder%C_%EL%_%YS%_ED%ED%.gdx' yre, DOI, U_nom, N_modules, P_PV, P_WT, En_ES, ChgP, DisP,
                                     P_grid, P_EL, N_EL_on, M_EL, M_RWS, M_MSR, MeOH_coal_hr
