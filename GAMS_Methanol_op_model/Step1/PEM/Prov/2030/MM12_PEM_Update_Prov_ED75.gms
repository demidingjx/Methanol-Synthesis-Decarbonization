...
eq_F_prov_a(emz(ln,jsx,js),h) .. F_prov(ln,jsx,js,h) * (1 - pln_loss(ln,js,jsx)) =L= U_prov_nom(ln,jsx,js) * 0.3;   // MW
eq_F_prov_b(emz(ln,jsx,js),h) .. F_prov(ln,js,jsx,h) * (1 - pln_loss(ln,js,jsx)) =L= U_prov_nom(ln,jsx,js) * 0.3; 
eq_F_prov_c(emz(ln,jsx,js),h) .. (F_prov(ln,js,jsx,h) + F_prov(ln,jsx,js,h)) * (1 - pln_loss(ln,js,jsx)) =L= U_prov_nom(ln,jsx,js) * 0.3; 
...
*全国集成度为50%
*eq_MSR_Gas .. sum((plt(js,i),h), M_RWS('Gas',js,i,h)) =E= DOI * sum((plt(js,i),h), M_MSR('Gas',js,i,h));
...
Execute_Unload '%Sfolder%P_%EL%_%YS%_ED%ED%.gdx' yre, DOI, U_nom, N_modules, P_PV, P_WT, En_ES, ChgP, DisP, P_city_PV, P_city_WT, P_city,
                                     U_city_nom,  P_grid, P_EL, N_EL_on, M_EL, M_RWS, M_MSR, MeOH_coal_hr
                                     CO2_coal, CO2_capt, CO2_pur, CO2_em, Capex_RE, FOM_RE, Capex_EL, FOM_EL, VOM_EL, Capex_ES, FOM_ES, VOM_ES, Capex_RWS, FOM_RWS, VOM_RWS
                                     VOM_Pcity, E_Purs, FSC_H2O, FSC_O2, FSC_CO2, FSC_CO2e, FSC_CO2p, FSC_CO2c, Capex, FOM, VOM, EPC,
