...
def plot_map(el, year, ed):
    ...
    for prov_type, key in [("AC", "prov_ac"), ("DC", "prov_dc")]:
        ...
        for v, lw in zip(vals, widths):
            ...
            for v, lw in zip(vals, widths):
                if v <= GW_bins_prov[1]:
                    sec = f"[{GW_bins_prov[0]}, {GW_bins_prov[1]}]"
                elif v <= GW_bins_prov[2]:
                    sec = f"({GW_bins_prov[1]}, {GW_bins_prov[2]}]"
                else:
                    sec = f"({GW_bins_prov[2]}, {GW_bins_prov[3]}]"
                print(f"Value: {v:.2f} GW, Linewidth: {lw:.2f}, 区间: {sec}")
    ...
    components = ['PV','WT','PEM','HST']
             
    colors = ["#d73027", "#fc8d59", "#fee08b", "#78afab", "#91cf60"]
