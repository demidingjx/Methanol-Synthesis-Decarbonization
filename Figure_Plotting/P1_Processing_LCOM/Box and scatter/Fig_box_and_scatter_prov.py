...
for year in years:
    for ei_pct in year_ei_levels[mode][year]:
        ei = ei_pct.rstrip('%')
        file_path = os.path.join(base_dir, f"ED{ei}", f"LCOM_{mode}_ED{ei}.xlsx")
        sheet_name = f"{year}_Prov_LCOM_j"
        try:
            df = pd.read_excel(file_path, sheet_name=sheet_name)
        except Exception as e:
            print(f"无法读取 {sheet_name} in {file_path}: {e}")
            continue

        df = df.rename(columns={df.columns[0]: 'Province', df.columns[1]: 'LCOM'})
        df['Year']  = str(year)  
        df['Group'] = f"{year}\n{ei}%"      
        all_data.append(df)
...
hex_colors = ["#87d392", "#54bd80", "#30ae88"] 
...
png_path = os.path.join(save_dir, f"{mode}_LCOM_BoxScatter.png")
pdf_path = os.path.join(save_dir, f"{mode}_LCOM_BoxScatter.pdf")
