...
$gdxin %Sfolder%C_%EL%_%ys%_%ED%.gdx
...
LCOM_city(j,i) = (sum(rg, Capex_RE(rg,j,i)) + Capex_EL(j,i)  + sum(es, Capex_ES(es,j,i))
                  + sum(rg, Fom_RE(rg,j,i)) + FOM_EL(j,i)  + sum(es, FOM_ES(es,j,i))
                  + VOM_EL(j,i) + sum(es, VOM_ES(es,j,i)) 
                  ...
                  + TAC_Msyn_city(j,i))/MeOH_Cap_yr(j,i);



$onEcho > howToWrite_C_2030.txt

Text = "City_LCOM"           rng = LCOM!A5
Par = LCOM.l                 rng = LCOM!B5 
Text = "City_RLCCE"           rng = RLCCE!A4
Par = RLCCE.l                rng = RLCCE!B4 

Text = "City_LCOM_j"          rng = City_RLCCE_j!A1
Text = "Value"                rng = City_RLCCE_j!B1 
Par = RLCCE_prov               rng = City_RLCCE_j!A2     rDim = 1

$offEcho

//记得对应技术、年份与ED
execute 'gdxxrw %Sfolder%C_%EL%_%ys%_%ED%.gdx output = %Lfolder%LCOM_city_%ys%_%EL%_%ED%.xlsx @howToWrite_C_2030.txt';


Execute_Unload 'LCOM_city_%EL%_%YR%.gdx' LCOM_city;
$onEcho > LCOM_city_2030.txt
Text = "Prov"                 rng = City_LCOM_j!A1
Text = "City"                rng = City_LCOM_j!B1
Text = "Value"                rng = City_LCOM_j!C1 

Par = LCOM_city               rng = City_LCOM_j!A2     rDim = 2
$offEcho
