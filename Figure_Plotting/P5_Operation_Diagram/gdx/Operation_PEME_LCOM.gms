...
*---------------------读数据----------------------------*
Parameter U_nom(j,ix,cb), P_PV(j,ix,h), P_WT(j,ix,h), P_EL(j,i,h), ChgP(es,j,i,h), DisP(es,j,i,h), M_RWS(csp,j,i,h)
          P_grid(j,i,h), MeOH_coal_hr(j,i,h), M_MSR(csp,j,i,h), En_ES(es,j,i,h)
          M_EL(csp,j,i,h);

$gdxin %Sfolder%S_%EL%_%ys%_%ED%.gdx
$load U_nom = U_nom.l, P_PV = P_PV.l, P_WT = P_WT.l, P_EL = P_EL.l, ChgP = ChgP.l, DisP = DisP.l, P_grid = P_grid.l, MeOH_coal_hr = MeOH_coal_hr.l, M_EL = M_EL.l, M_RWS = M_RWS.L, M_MSR = M_MSR.l, En_ES = En_ES.l
$gdxin
...
Tol_ChgP_tank(h) = sum((j,i),ChgP('%TN%',j,i,h)) * 0.03333; 
Tol_DisP_tank(h) = sum((j,i),DisP('%TN%',j,i,h)) * 0.03333;
Tol_En_ES_LiB(h) = sum((j,i),En_ES('LiB',j,i,h)) * 10E-3;
Tol_En_ES_tank(h) = sum((j,i),En_ES('%TN%',j,i,h)) * 0.03333;
...
VOM_h(h) = sum((yre,js,i), Tech_VOM('%el%',yre) * P_EL(js,i,h))
         + sum((yre,js,i), Tech_VOM('RWS',yre) * M_RWS('Gas',js,i,h))
...
FSC_H2O_t(h) = (uH2O * sum((js,i), (M_EL('H2O',js,i,h) - M_RWS('H2O',js,i,h)))) / ths;
