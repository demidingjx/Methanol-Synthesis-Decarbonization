...
Parameter   Total_ems;
Total_ems = (CO2_PV + CO2_WT +CO2_grid + CO2_ems)/1e6;
Display Total_ems


* =======================Decarbonization cost=================================*
Parameter Decar_cost;
RLCCE = (CO2_PV + CO2_WT + CO2_grid + CO2_ems)/sum((j,i), MeOH_Cap_yr(j,i) * ths) +  CO2CtM * (1 - %DOI%);

Decar_cost = DLCOR / MAX(0.00001, (CO2CtM - RLCCE)) ;
*Decar_cost = RLCCE;
Display Decar_cost
* ------------------Write in Excel -------------------*
Execute_Unload 'All_data_%EL%_%YR%_%ED%.gdx' Eco_cost, CO2_cost, Decar_cost, RLCCE, DLCOR;
$onEcho > Eco_cost.txt
...
Text = 'DLCOR'          rng = LCCR_detial!A1
Par = DLCOR             rng = LCCR_detial!B1

Text = 'LCE'          rng = LCCR_detial!A2
Par = RLCCE             rng = LCCR_detial!B2
