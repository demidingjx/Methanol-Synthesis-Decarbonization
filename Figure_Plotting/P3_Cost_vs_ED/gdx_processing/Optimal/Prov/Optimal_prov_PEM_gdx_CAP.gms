...
$Set Sfolder D:\GAMS_optimization\Statewide\Updated_Gams\GDX2_W52_OP\%EL%\%YR%\%ED%\
...
Set cb / PV, WT, %EL%, Lib, %TN%, TNS, Grid, H2O, O2, CO2, OM_coal, Coal  /
    cbd / PV_CAP, PV_OM, WT_CAP, WT_OM, %EL%_CAP, %EL%_OM, Lib_CAP, Lib_OM, %TN%_CAP, %TN%_OM, RWS_CAP, RWS_OM, TNS, Grid, H2O, O2, CO2, OM_coal, Coal, OM_Msyn  /
    eco / CAPEX, FOM, VOM/
    rg(cb) / PV, WT /
    es(cb) / Lib, %TN% /
    ln / AC, DC /
...
$gdxin %Sfolder%W52_%TP%_%EL%_%YR%_%ED%.gdx
...
TNS_prov(j) = sum((ix,i),VOM_Pcity(j,ix,i));
...
*Decar_cost = DLCOR / (CO2CtM - RLCCE);
Decar_cost = RLCCE;
Display DLCOR, RLCCE, Decar_cost
* ------------------Write in Excel -------------------*
...
Text = "Unit Emission"   rng = Decarbonization!A1
Par = Decar_cost     rng = Decarbonization!B1

$offEcho

//记得对应技术、年份与ED
execute 'gdxxrw All_data_%EL%_%YR%_%ED%.gdx output = %Cfolder%TCB_CAP_%TP%_%EL%_%YR%_%ED%.xlsx @Eco_cost.txt';
