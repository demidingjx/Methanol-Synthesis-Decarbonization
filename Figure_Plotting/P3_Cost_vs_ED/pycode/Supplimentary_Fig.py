...
def load_data(el):
    ...
    for year in year_ei_levels[el]:                          
        ...
        for ei_str in year_ei_levels[el][year]:                   
            ei = ei_str.rstrip('%')                             
        ...
        df_cb[year] = pd.concat(cb_parts, axis=1)                                   
        df_lcce[year] = lcce_dict                                
...
def plot_combined(el, df_cb, df_lcce, ax1, ax2=None):
    ...
    for i, year in enumerate(year_ei_levels[el]):
        ei_list = [e.rstrip('%') for e in year_ei_levels[el][year]]                 
    ...
    for i, year in enumerate(year_ei_levels[el]):                     
        ei_list = [e.rstrip('%') for e in year_ei_levels[el][year]]                
        base = i * per_year
        for k, ei in enumerate(ei_list):                               
            mid = base + k * group_size + 1                      
