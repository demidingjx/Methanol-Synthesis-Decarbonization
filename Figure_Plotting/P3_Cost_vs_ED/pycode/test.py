    ...
    for year in year_ei_levels[el]:          # 2030/2040/2050
        ...
        for ei_str in year_ei_levels[el][year]:   # ['25%','100%']
            ...
            cb_parts.append(tmp[['City','Prov','State']].rename(columns={
                'City': f'C{ei}', 'Prov': f'P{ei}', 'State': f'S{ei}'
            }))
            ...
            if {'City','Prov','State'}.issubset(t.columns):
                ...
                row = t.iloc[row_idx]
                lcce_dict[f'C{ei}'] = _sf(row['City'])
                lcce_dict[f'P{ei}'] = _sf(row['Prov'])
                lcce_dict[f'S{ei}'] = _sf(row['State'])
            else:
                ...
                lcce_dict[f'C{ei}'] = _sf(t2.loc['City', col])
                lcce_dict[f'P{ei}'] = _sf(t2.loc['Prov', col])
                lcce_dict[f'S{ei}'] = _sf(t2.loc['State', col])
...
def plot_combined(el, df_cb, df_lcce, ax1, ax2=None):
    ...
    for i, year in enumerate(year_ei_levels[el]):
        ...
        labels = [f'{r}{ei}' for ei in ei_list for r in ['C','P','S']]  # C25,P25,S25,C100,P100,S100
        ...
        for j, lab in enumerate(labels):
            ...
            if np.isfinite(num):
                ...
                ax1b.text(x_pos, num + 30, f"{num:.1f}", ha='center', va='bottom', color='blue')
    ...
    tick_lbls = [s[0] if s != '' else '' for s in x_labels]   # 'C25'→'C'
    ax1.set_xticklabels(tick_lbls, rotation=0, fontsize=16, ha='center')
...
if __name__ == '__main__':
    ...
    fig, axes = plt.subplots(2, 1, figsize=(18, 18), sharey='row',
                             gridspec_kw={'hspace': 0.1, 'wspace': 0.2})  # <<< 修改：改为 2x1
